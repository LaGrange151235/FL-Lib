...
class AggregationService(Service):
    ...
    def exposed_aggregate(self, rank, round_id, tensor_list, nova_tau):
        ...
        self.nova_tau_buffer.append(nova_tau)
        if sum(self.arrived) == int(self.client_number * self.C):
            ...
            for tensor_id, tensor_content in enumerate(self.tensor_buffer[0]):
                ...
                for i in range(len(self.tensor_buffer)):
                    sum_tensor+= (self.dataset_size_buffer[i]/sum(self.dataset_size_buffer))*self.tensor_buffer[i][tensor_id]/self.nova_tau_buffer[i]
                norm_weight = 0
                for i in range(len(self.tensor_buffer)):
                    norm_weight += self.dataset_size_buffer[i] * self.nova_tau_buffer[i] / sum(self.dataset_size_buffer)
                aggregated_tensor = norm_weight * sum_tensor
