...
class AggregationService(Service):
    def __init__(self, args):
        ...
        # TODO: For SCAFFOLD
        self.C_tensor_list = [torch.zeros_like(tensor) for tensor in model_2_tensor_list(self.model)]
        self.Delta_C_i_buffer = []
    ...
    def exposed_C_tensor_list(self, rank):
        C_tensor_list = copy.deepcopy(self.C_tensor_list)
        self.logging("send C_tensor_list to rank %d." % (rank))
        return C_tensor_list
    
    def exposed_start(self):
        return self.start_train
    
    def exposed_aggregate(self, rank, round_id, tensor_list, Delta_C_i):
        tensor_list = rpyc.classic.obtain(tensor_list)
        Delta_C_i = rpyc.classic.obtain(Delta_C_i)
        ...
        self.Delta_C_i_buffer.append(Delta_C_i)
        if sum(self.arrived) == int(self.client_number * self.C):
            ...
            aggregated_C_tensor_list = []
            for tensor_id, tensor_content in enumerate(self.Delta_C_i_buffer[0]):
                sum_tensor = torch.zeros(tensor_content.size())
                for i in range(len(self.Delta_C_i_buffer)):
                    sum_tensor+=self.Delta_C_i_buffer[i][tensor_id]
                aggregated_C_tensor = sum_tensor/(len(self.Delta_C_i_buffer))
                aggregated_C_tensor_list.append(aggregated_C_tensor)
            self.C_tensor_list = aggregated_C_tensor_list
