...
class Client:
    def __init__(self, local_profile, sync_profile):
        ...
        # TODO: For MOON
        self.moon_init_model = None
        self.moon_prev_local_model = None
        self.moon_temperature = 0.5
    ...
    def train(self):
        ...
        self.moon_init_model = copy.deepcopy(self.model)
        self.moon_prev_local_model = copy.deepcopy(self.model)
        ...
        while epoch_id < self.max_epoch:
            ...
            for step, (b_x, b_y) in enumerate(self.training_dataloader):
                ...
                output, represent = self.model(b_x)
                output2, represent2 = self.moon_init_model(b_x)
                output3, represent3 = self.moon_prev_local_model(b_x)
                posi = cos(represent, represent2)
                nega = cos(represent, represent3)
                logits = posi.reshape(-1,1)
                logits = torch.cat((logits, nega.reshape(-1,1)), dim=1)
                logits /= self.moon_temperature
                labels = torch.zeros(b_x.size(0))
                if CUDA:
                    labels = labels.cuda().long()
                else:
                    labels = labels.long()

                loss1 = self.loss_func(output, b_y)
